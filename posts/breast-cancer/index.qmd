...
we are going to explore the Breast Cancer Wisconsin Data Set, which contains 30 features extracted from a digitised image of a fine needle aspirate of 569 breast masses, and each breast mass is associated to a diagnosis: malignant (M) or benign (B). So we have $n=569$ observations of $d = 30$ covariates, denoted as $X \in \mathbb{R}^{n \times d}$, along with the diagnosis. We have 212 malignant and 357 benign cases in the dataset, which is well-balanced. Our objection is to classify whether a breast mass is malignant or benign based on the extracted features, so our response of interest is then a binary variable denoted as $y$. We define $y$ to be 1 if the breast mass is malignant, and 0 otherwise. So we have $y \in \{0, 1\}$. All the features in this dataset have been normalised before interpretation.
...
These procedures are done by using package \verb\|sklearn\|. We use \verb\|StratifiedKFold\| for the cross-validation, \verb\|LogisticRegression, KNeighborsClassifier, GaussianNB\| for the classification model, and \verb\|cross_val_score\| to assess the accuracy and stability. We choose $K=3$, $K=5$, and $K=7$ for the numbers of neighbours for the k-nearest neighbours classifier.
...
\begin{table}[ht]
    ...
    \begin{tabular}{|c|c|c|c|}
        ...
        KNN($K=3$) & 0.9683 & 0.0154 \\
        KNN($K=5$) & 0.9666 & 0.0140 \\
        KNN($K=7$) & 0.9596 & 0.0142 \\
        NaiveBayes & 0.9297 & 0.0199 \\ 
        \hline
    \end{tabular}
    \caption{The mean accuracy score and standard deviation for the three classifiers, Logistic regression, KNN (for the number of neighbours = 3, 5, 7), and Naive Bayes.}
    \label{tab:classifier_perform}
\end{table}
```
